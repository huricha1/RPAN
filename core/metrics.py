...
def relabeling_strategy(df, params):
    ...
    for t in thresholds:
        ...
        #     print("t: ", t)
        # df_sort:7200*8
        y_true, y_pred, thred,diff = predict_labels(df_sort, t)
        df_sort
        ...
        if f1 > best_f1:
            ...
            best_diff=diff
    best_prediction = np.array(best_predictions)
    print("best_thred:",best_thred)
    np.savetxt(f"experiments/service0_predict_label.txt", best_prediction, delimiter=',')
    # np.savetxt(f"experiments/service0_diff.txt", best_diff, delimiter=',')
    np.savetxt(f"experiments/service0_y_true.txt", y_true, delimiter=',')
...
def predict_labels(df_sort, num):
    ...
    diff = df_sort['differ']
    return y_true, y_pred, thred,diff
