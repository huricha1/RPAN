...
class PrepareTimeData:
    ...
    def read_dataset(self, data_path, data_name):
        ...
        if data_name.upper().find('MSL') != -1:
            ...
            self.get_dataset(data_path, cols)
        elif data_name.find('service0') != -1:
            cols = [-1]
            self.get_dataset(data_path, cols)
        elif data_name.find('omi-2') != -1:
            cols = [-1]
            self.get_dataset(data_path, cols)
    ...
    def get_data_by_insert_normal(self):
        ...
        np.savetxt(f"experiments/service0_df_pre_label.txt", df_pre_label, delimiter=',')
    ...
    def mutation_point(self, df):
        ...
        start_index = 0
        end_index = size
        # 计算的是 df_pre_label 数据框中 'value' 列的样本方差
    ...
    def insert_normal(self, data):
        ...
        # print(self.get_index(ano_indexes))
        interval = int(self.size / self.base)
        # print(self.size,self.base)
        ano_len = 2
